...
async def example_provider_usage():
    ...
    print(f"Default provider: {config.default_config.llm_provider.provider}")
    ...
    for provider in available_providers:
        try:
            # Check if provider is available and show its basic info
            if provider in available_providers:
                print(f"{provider.upper()}:")
                print("  Available: Yes")
                default_model = (
                    config.default_config.llm_provider.model
                    if provider == config.default_config.llm_provider.provider
                    else "N/A"
                )
